...
module.exports = {
    ...
    sendTicket: async function (order, type) {
        ...
        let content = Utils.encodeBase64(JSON.stringify(type == "flight.national.afterservice.ticketNoUpdate" ? {
            "orderNo": order.groupId,
            "serviceOrderNo": change ? change.groupId : "",
            "serviceOrderType": change ? 1 : -1,
            "serviceOrderStatus": change ? status[order.orderStatus] : "17",
            "ticketPassengers": [{
            ...
            "serviceOrderNo": change ? change.groupId : order.groupId,
            "serviceOrderType": change ? 1 : 0,
            "serviceOrderStatus": change ? status[order.orderStatus] : "17",
            "dataExt": {
